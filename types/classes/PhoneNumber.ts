 ...
 * @returns number
 */
function removeNonDigitsAndAddPrefix(value: string | number, prefix: string | number): number {
 ...
 * @returns string
 */
function formatPhoneNumber(value: string | number, prefix: string | number): string {
  let alt = `${value}`.replace(/\D/g, '');
  if (alt.length === 10) alt = prefix + alt;
